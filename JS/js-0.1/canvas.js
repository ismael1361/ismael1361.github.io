(function(win, root){
  ...
  win.Canvas = function(){
    if(arguments.length > 0 && typeof arguments[0] == 'string' && arguments[0] instanceof win.Get){
      ...
      this.canvas = document.createElement('canvas');
      arguments[0] = typeof arguments[0] == "boolean" ? arguments[0] : true;
      if(arguments[0] == true){win.Get(document.body).prepend(Get(this.canvas));}
    }
  ...
  win.Canvas.prototype = {
    "CoordinateToLength" : function(a, b){
      return (Math.round(a) + (Math.floor(b) * this.width())) * 4;
    },

    "LengthToCoordinate" : function(a){
      var b = 0, c = 0;
      c = Math.floor(Math.floor(a / this.width()) / 4);
      b = Math.floor(a / 4) - Math.floor(c * this.width());
      return [b, c];
    },
    ...
    "clear" : function(a, b, c, d){
      a = typeof a != 'number' ? 0 : a < 0 ? 0 : a > this.width() ? this.width() : a;
      b = typeof b != 'number' ? 0 : b < 0 ? 0 : b > this.height() ? this.height() : b;
      c = typeof c != 'number' ? this.width() : c < 0 ? 0 : c > this.width() ? this.width() : c;
      d = typeof d != 'number' ? this.height() : d < 0 ? 0 : d > this.height() ? this.height() : d;
    ...
    "pixelForEach" : function(a){
      ...
      for(var c=0; c<this.height(); c++){
        for(var d=0; d<this.width(); d++){
          var e = this.CoordinateToLength(d, c), f = [];
    ...
    "filterBlur" : function(a, b, c, d, e, c_, v_){
      ...
      if(Array.isArray(c_)){
        var tmp = [];
        for(var i=0; i<4; i++){
          tmp[i] = typeof c_[i] == "number" && c_[i] < 256 ? c_[i] : 255;
        }
        c_ = tmp;
      }else if(typeof c_ == "object"){
        var tmp = [];
        for(var i=0; i<4; i++){
          var k = i == 0 ? "r" : i == 1 ? "g" : i == 2 ? "b" : i == 3 ? "a" : "_c_";
          tmp[i] = typeof c_[k] == "number" && c_[k] < 256 ? c_[k] : 255;
        }
        c_ = tmp;
      }else{
        c_ = null;
      ...
      v_ = typeof v_ == "number" ? (v_ > 1 ? (v_ < 255 ? (v_/255) : 1) : v_) : 0;
      ...
      this.pixelForEach(function(a){if(a.r == 0 && a.g == 0 && a.b == 0 && a.a == 0){return [255, 255, 255];} return a;});
      ...
      for(var f=0; f<2; f++){
        ...
        for(var x=0; x<img.width; x++){
          ...
          for(var y=0; y<img.height; y++){
            ...
            if(c_ != null){
              var m_ = f == 1 ? Math.abs(Math.random() > 0.5 ? (c_[3]/255) : ((c_[3]/255) - v_)) : Math.abs(c_[3]/255);
              px[yi+0] = Math.floor((px[yi+0] + (c_[0] * m_))/(2-(1-m_)));
              px[yi+1] = Math.floor((px[yi+1] + (c_[1] * m_))/(2-(1-m_)));
              px[yi+2] = Math.floor((px[yi+2] + (c_[2] * m_))/(2-(1-m_)));
            }
            if(x == 0){
              var p = y+(a+1);
